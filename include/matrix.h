...
// std::ostream &operator << (std::ostream &out, const std::tuple<uint, uint, uint> & tp){
//     out << '(' << get<0>(tp) << ',' << get<1>(tp) << ',' <<get<2>(tp) <<')'<< std::endl;
// return out;
// }
// std::ostream &operator << (std::ostream &out, const Matrix<std::tuple<uint,uint,uint> &m)
// {
...
//              out << '(' << get<0>(m(i,j)) << ',' << get<1>(m(i,j)) << ',' <<get<2>((i,j)) <<')'<< std::endl;
//   //          out << m(i, j) << " ";
//         }
//         out << std::endl;
//     }
...
// taken from http://stackoverflow.com/questions/9247723/overloading-operator-for-stdtuple-possible-simplications
